...
{
#if TIMER_USE_MC_PWM
    YT_MCPWM->TMR0_CON |= BIT(10); //清pending
#else
    YT_TIMERx->CON |= BIT(14);
#endif
    u8 i, led_idle = 0;
...
void mc_clk_init(void)
{
    ...
    YT_MCPWM->TMR0_CON = BIT(10); //清pending,清其他bit
    YT_MCPWM->MCPWM_CON0 = 0;
    ...
    YT_MCPWM->TMR0_CNT = 0;
    YT_MCPWM->TMR0_PR = prd_cnt;
    YT_MCPWM->TMR0_CON |= index << 3; //分频系数

    request_irq(IRQ_MCTMRX_IDX, 3, soft_pwm_led_isr, 0);

    YT_MCPWM->TMR0_CON |= BIT(8);  //允许定时溢出中断
    YT_MCPWM->TMR0_CON |= BIT(0);  //递增模式

    YT_MCPWM->MCPWM_CON0 |= BIT(8); //只开mc timer 0
    log_info("prd_cnt:%d,index:%d,t0:%x,MCP:%x\n", prd_cnt, index, YT_MCPWM->TMR0_CON, YT_MCPWM->MCPWM_CON0);
    log_info("lsb:%d\n", clk_get("lsb"));
#else
    u32 timer_clk;
    u32 prd_cnt;
    ...
    YT_TIMERx->PRD = prd_cnt;
#if (TCFG_CLOCK_SYS_SRC == SYS_CLOCK_INPUT_PLL_RCL)
    YT_IOMAP->CON0 |= BIT(TIMERX_IOS_BIT);
    YT_TIMERx->CON = (index << 4) | BIT(0) | BIT(2);//省晶振使用pll12m为时钟源
#else
    YT_TIMERx->CON = (index << 4) | (0b10 << 2) | (0b01 << 0);//OSC时钟
#endif
#endif
}
