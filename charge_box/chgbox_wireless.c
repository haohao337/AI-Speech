...
static void wl_timer2_open(void)
{
    ...
    YT_TIMER2->CNT = 0;
    YT_TIMER2->PRD = prd_cnt;
    request_irq(IRQ_TIME2_IDX, 3, wl_timer2_isr, 0);
#if (TCFG_CLOCK_SYS_SRC == SYS_CLOCK_INPUT_PLL_RCL)
    YT_TIMER2->CON = (index << 4) | BIT(0); //lsb clk
#else
    YT_TIMER2->CON = (index << 4) | BIT(0) | BIT(3); //osc clk
#endif
    is_wl_comm_active = 1;
}
