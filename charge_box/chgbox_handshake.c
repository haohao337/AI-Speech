...
static void chgbox_timer_delay_ms(u8 ms)
{
    ...
    YT_TIMER2->CNT = 0;
#if (TCFG_CLOCK_SYS_SRC == SYS_CLOCK_INPUT_PLL_RCL)
    YT_TIMER2->PRD = ms * clk_get("lsb") / (2 * 1000);
    YT_TIMER2->CON = BIT(0) | BIT(6) | BIT(14); //系统时钟,lsb, div 2
#else
    YT_TIMER2->PRD = ms * clk_get("timer") / 1000;
    YT_TIMER2->CON = BIT(0) | BIT(3) | BIT(14); //1分频,osc时钟,24m，24次就1us
#endif
    while (!(YT_TIMER2->CON & BIT(15))); //等pending
    YT_TIMER2->CON = 0;
}
...
void chgbox_timer_delay_us(u8 us)
{
    ...
    YT_TIMER2->CNT = 0;
#if (TCFG_CLOCK_SYS_SRC == SYS_CLOCK_INPUT_PLL_RCL)
    YT_TIMER2->PRD = delay_tap[hs_cur_sys_clk][us];//这里不允许做乘除法
    YT_TIMER2->CON = BIT(0) | BIT(6) | BIT(14); //系统时钟,lsb, div 2, 必须为24M
#else
    YT_TIMER2->PRD = delay_tap[hs_cur_sys_clk][us];
    YT_TIMER2->CON = BIT(0) | BIT(3) | BIT(14); //1分频,osc时钟,24m，24次就1us
#endif
    while (!(YT_TIMER2->CON & BIT(15))); //等pending
    YT_TIMER2->CON = 0;
