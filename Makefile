...
# make clean 清除编译临时文件
# make CUSTOMER=xxxx 编译指定客户xxxx; 不加CUSTOMER, 编译board_yt212x

LCUSTOMER = $(shell echo $(CUSTOMER) | tr '[A-Z]' '[a-z]')
UCUSTOMER = $(shell echo $(CUSTOMER) | tr '[a-z]' '[A-Z]')
override CUSTOMER := $(LCUSTOMER)
# $(info CUSTOMER:$(CUSTOMER))

# 工具路径设置
ifeq ($(OS), Windows_NT)
# Windows 下工具链位置
TOOL_DIR := C:/YT/pi32/bin
CC    := clang.exe
CXX   := clang.exe
...
SYS_LIB_DIR := C:/YT/pi32/pi32v2-lib/r3
SYS_INC_DIR := C:/YT/pi32/pi32v2-include
...
# Linux 下工具链位置
ifdef YT_TOOLCHAIN
TOOL_DIR := $(YT_TOOLCHAIN)/pi32v2/bin
else
TOOL_DIR := tools/tool_chain/pi32v2/bin
endif
CC    := clang
CXX   := clang++
LD    := lto-wrapper
...
CUSTOMER_ARR := $(space)$(shell ls apps/soundbox/board/br23/customers/customized/) 
$(info Supported customers:$(CUSTOMER_ARR))

ifeq ($(findstring $(space)$(CUSTOMER) ,$(CUSTOMER_ARR)),)
${error Unsupported customer:$(CUSTOMER). Please enter the correct customer name!}
endif
endif
...
INCLUDES := \
	...
	-Iapps/common/third_party_profile/yuntai \
	-Iapps/common/third_party_profile/yuntai/trans_data_demo \
	-Iapps/common/third_party_profile/yuntai/online_db \
	-Iapps/common/third_party_profile/yuntai/YT_rcsp \
	-Iapps/common/third_party_profile/yuntai/YT_rcsp/rcsp_updata \
	-Iapps/common/third_party_profile/yuntai/YT_rcsp/bt_trans_data \
	-Iapps/common/third_party_profile/yuntai/YT_rcsp/adv_rcsp_protocol \
	-Iapps/common/third_party_profile/yuntai/YT_rcsp/adv_app_setting \
	...
	-Iapps/soundbox/board/br23/include \
	-Icpu/br23 \
	...
	-Iapps/soundbox/board/br23/board_yt212x/lib_allinone/include \
	-I$(SYS_INC_DIR) \
...
c_SRC_FILES := \
	...
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_app_setting/adv_bt_name_setting.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_app_setting/adv_key_setting.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_app_setting/adv_led_setting.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_app_setting/adv_mic_setting.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_app_setting/adv_time_stamp_setting.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_app_setting/adv_work_setting.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_rcsp_protocol/rcsp_adv_opt.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/adv_rcsp_protocol/rcsp_adv_tws_sync.c \
	apps/common/third_party_profile/yuntai/YT_rcsp/rcsp_updata/rcsp_adv_user_update.c \
	apps/common/third_party_profile/yuntai/hid_user.c \
	apps/common/third_party_profile/yuntai/le_client_demo.c \
	apps/common/third_party_profile/yuntai/multi_demo/le_multi_client.c \
	apps/common/third_party_profile/yuntai/multi_demo/le_multi_common.c \
	apps/common/third_party_profile/yuntai/multi_demo/le_multi_trans.c \
	apps/common/third_party_profile/yuntai/online_db/online_db_deal.c \
	apps/common/third_party_profile/yuntai/online_db/spp_online_db.c \
	apps/common/third_party_profile/yuntai/trans_data_demo/le_trans_data.c \
	apps/common/third_party_profile/yuntai/trans_data_demo/spp_trans_data.c \
	...
	apps/common/third_party_profile/tuya_protocol/port/YT_to_tuya_ble_port_peripheral.c \
	...
	apps/soundbox/board/br23/board_yt212x/usr/src/user_fan.c \
	cpu/br23/adc_api.c \
...
ifeq ($(CUSTOMER), )
INCLUDES += \
	-Iapps/soundbox/board/br23/board_yt212x \

c_SRC_FILES += \
	apps/soundbox/board/br23/board_yt212x/board_yt212x.c \
	apps/soundbox/board/br23/board_yt212x/key_table/adkey_table.c \
	apps/soundbox/board/br23/board_yt212x/key_table/iokey_table.c \
	apps/soundbox/board/br23/board_yt212x/key_table/irkey_table.c \
	apps/soundbox/board/br23/board_yt212x/key_table/rdec_key_table.c \
	apps/soundbox/board/br23/board_yt212x/key_table/touch_key_table.c \
...
LFLAGS := \
	...
	include_lib/liba/br23/lib_dns.a \
...
# user options
ifeq ($(LAIS_REPLAY),y)
DEFINES += -DLAIS_REPLAY_EN
endif

ifeq ($(LAIS_MFT),y)
DEFINES += -DUSER_MFT_EN
# enable usb and hid debug 
HID_DEBUG := y
endif

ifeq ($(LAIS_AGC),y)
DEFINES += -DLAIS_AGC_EN
endif

HID_DEBUG ?= n
ifneq ($(HID_DEBUG),n)
DEFINES += -DUSER_USB_HID_DEBUG
endif

ifneq ($(CUSTOMER), )
DEFINES += -DCONFIG_BOARD_YT212X_CUS_$(UCUSTOMER)
DEFINES += -DCONFIG_CUSTOMER="$(CUSTOMER)"
endif

# customer
ifeq ($(CUSTOMER), )
include apps/soundbox/board/br23/board_yt212x/usr/Makefile
include apps/soundbox/board/br23/board_yt212x/sdk/Makefile
else
include apps/soundbox/board/br23/customers/Makefile
endif
...
.PHONY: all clean pre_build post_build help

all: pre_build $(OUT_ELF) post_build
	$(QUITE) $(RUN_POST_SCRIPT) sdk
ifneq ($(MISC_POST_SCRIPT),)
	$(QUITE)  bash $(MISC_POST_SCRIPT)
endif
	

post_build: $(OUT_ELF)
	@echo +POST-BUILD
ifeq ($(LAIS_RELEASE_VER), n)
	$(RM) $(LAIS_LIB)
#	$(RM) $(LITE_RES_LIB)
	$(RM) $(LITE_CORE_LIB)
	$(RM) $(LITE_CORE_SSPE_LIB)
	$(RM) $(LITE_CORE_SEVC_LIB)

	$(AR) r $(LAIS_LIB) $(LAIS_OBJS)
#	$(AR) r $(LITE_RES_LIB) $(LITE_RES_OBJS)
	$(AR) r $(LITE_CORE_LIB) $(LITE_CORE_OBJS)
ifeq ($(LAIS_ALL_IN_ONE), y)
	$(AR) x $(LITE_APP_LIB)
	$(AR) r $(LITE_CORE_LIB) *.c.o
	$(RM) *.c.o
endif

ifeq ($(LAIS_USE_AEC), y)
	$(AR) x $(LITE_SSPE_LIB)
	$(AR) r $(LITE_CORE_SSPE_LIB) $(LITE_CORE_SSPE_OBJS) *.c.o
	$(RM) *.c.o
endif

ifeq ($(LAIS_USE_SEVC), y)
	$(AR) x $(LITE_SEVC_LIB)
	$(AR) r $(LITE_CORE_SEVC_LIB) $(LITE_CORE_SEVC_OBJS) *.o
	$(RM) *.o
endif
...
help:
	@echo "Targets:\n"
	@echo "all:		-	default target, build every thing"
	@echo "clean:		-	clean intermediate objects"
	@echo "help:		-	show help info"
	@echo "pre_build:	-	prepare for ELF building"
	@echo "post_build:	-	custom processing after ELF building"
	@echo "\nConfiguration:\n"
	@echo "LAIS_RELEASE_VER:	-	default is 'y' for customer, set to 'n' for AIS developer"
	@echo "LAIS_REPLAY:		-	default is 'n', set to 'y' to enable LAIS replay feature support"
	@echo "LAIS_MFT:		-	default is 'n', set to 'y' to enable manufature test support"
	@echo "LAIS_AGC:		-	default is 'n', set to 'y' to enable simple auto mic gain control"
	@echo "CUSTOMER:		-	Default build board_ yt212x, set to 'xxxx' to build customer 'xxxx'"

pre_build:
ifneq ($(CUSTOMER),)
	$(info Build customer:$(CUSTOMER))
endif
	@echo +PRE-BUILD
