...
static u32 hid_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *req)
{
    ...
    log_debug("hid_itf_hander usb_id:%d, bRequestType:%d, bRequest:%d\n", usb_id, bRequestType, req->bRequest);
    switch (bRequestType)
    {
    case USB_TYPE_STANDARD:
        ...
        case USB_REQ_GET_DESCRIPTOR:
            log_debug("hid_itf_hander HIBYTE(req->wValue):%d\n", HIBYTE(req->wValue));
            ...
            case USB_HID_DT_REPORT:
                log_debug("hid_itf_hander req->wIndex:%d\n", req->wIndex);
...
#ifdef USER_USB_HID_DEBUG
            user_cmd_hid_usb_set_setup_recv(usb_device);
#else
