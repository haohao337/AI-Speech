...
#if (SMART_BOX_EN && YT_SMART_BOX_EXTRA_FLASH_OPT)
...
/* const char updata_file_name[] = "/UPDATA/YT_692X.BFU"; */
...
#if (TCFG_UI_ENABLE&&(CONFIG_UI_STYLE == STYLE_YT_LED7))
    u8 count = 0;
...
#if(YT_EARPHONE_APP_EN && RCSP_UPDATE_EN)
        u8 clear_update_flag = 0;
        vm_write(VM_UPDATE_FLAG, &clear_update_flag, 1);
#endif
#if(YT_SMART_BOX_EXTRA_FLASH_OPT && RCSP_UPDATE_EN)
