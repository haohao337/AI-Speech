...
#define local_uart_set_dir(mode)           uart_set_dir(mode)
#define local_uart_write(data, len)        uart_update_write(data, len)
#define local_uart_set_baud(baudrate)      uart_update_set_baud(baudrate)
#define local_uart_close_deal()            uart_close_deal()
#define local_uart_hw_init(update_cfg, cb) uart_hw_init(update_cfg, cb)
#else
#include "user_uart_update.h"
#define local_uart_set_dir(mode)           user_uart_update_set_dir(mode)
#define local_uart_write(data, len)        user_uart_update_write(data, len)
#define local_uart_set_baud(baudrate)      user_uart_update_set_baud(baudrate)
#define local_uart_close_deal()            user_uart_update_close_deal()
#define local_uart_hw_init(update_cfg, cb) user_uart_update_hw_init(update_cfg, cb)
#endif
...
static bool uart_send_packet(u8 * buf, u16 length) {
    ...
    local_uart_set_dir(0);//设为输出
    put_buf((u8 *)&protocal_frame, length + SYNC_SIZE);
    local_uart_write((u8 *)&protocal_frame, length + SYNC_SIZE);
    local_uart_set_dir(1);
    return ret;
}
...
void uart_update_init(uart_update_cfg * cfg) {
    ...
    local_uart_hw_init(update_cfg, uart_data_decode);
    printf(">>>%s\n", __func__);
}
...
static void clock_critical_exit(void) {
    local_uart_set_baud(update_baudrate);
}
