...
void ac3433_uart_isr_callback(u8 uto_buf, void *p, u8 isr_flag)
{
    ...
    if (isr_flag == UART_ISR_TYPE_WRITE_OVER) {
        YT_UART0->CON0 &= ~BIT(2);     //disable tx isr
        uart_tx_dac();
    }
}
...
s32 ac3433_uart_init(u32 baud)
{
    ...
    YT_IOMAP->CON3 &= ~BIT(3);
    YT_IOMAP->CON1 &= ~(BIT(8) | BIT(9) | BIT(10) | BIT(11));	//output channel0 ->UT0_TX
    ...
    YT_UART0->BAUD = (UART_CLK / baud) / 3 - 1;
    YT_UART0->CON0 = BIT(13) | BIT(12) | BIT(10) | BIT(6) | BIT(4) | BIT(0);
